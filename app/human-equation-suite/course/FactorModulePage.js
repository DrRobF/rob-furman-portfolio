...
export default function FactorModulePage({ module }) {
  ...
  return <main className="section section-light"><div className="container"><HumanEquationNav />
    <section className='hes-awareness-hero hes-awareness-hero-premium top-space-sm'>
    ...
    <section className='top-space-sm card'><h2>How this module works</h2><p>These moments are not designed to find one perfect answer. They are designed to reveal what your leadership protects first under pressure: speed, clarity, emotional safety, structure, relationship, or authority.</p><p>As you respond, the system looks for patterns in how you interpret pressure. The goal is not perfection. The goal is awareness under load.</p></section>
    <section className='top-space-sm card'><h2>Interaction moments</h2><p>Choose the response you would most likely move toward under pressure. There may be more than one defensible answer. Your choices help reveal your first leadership instinct when tension rises.</p>{interactions.map((q,idx)=><article key={idx} className='top-space-sm'><h3>{q.prompt}</h3><p className='hes-interaction-context'>School pressure context: you are making a live leadership decision while people are watching for clarity, fairness, and steadiness.</p><div className='button-grid'>{q.options.map((o,i)=><button key={o.label} className={`button ${answers[idx]===i?'primary':'secondary'}`} onClick={()=>setAnswers((p)=>({...p,[idx]:i}))}>{o.label}</button>)}</div>{answers[idx]!==undefined&&(() => { const selected=q.options[answers[idx]]; const lens=signalFeedback[selected.signal]||signalFeedback.reflective; return <p><strong>Feedback:</strong> {selected.feedback} This choice protects {lens.protects}. It may cost {lens.cost}. Staff and families may feel {lens.others}. Pattern clue: {lens.pattern}.</p>; })()}</article>)}</section>
