...
export default function HumanEquationDashboardPage() {
  ...
  return <section className="section section-light"><div className="container"><LanguageSwitcher />
    ...
    {activeTab === 'urban' && <div className="top-space card project-card">{!urbanReport ? <><h2>Urban Student Report</h2><p>Urban Student Simulation has not been completed yet.</p><Link href="/day-in-the-life-urban-student" className="button secondary">Launch Urban Student Simulation</Link></> : <><h2>Urban Student Behavioral Evidence</h2><p><strong>Date completed:</strong> {urbanReport.completedAt ? new Date(urbanReport.completedAt).toLocaleString('en-US') : 'Unavailable'}</p><p>{urbanReport.evidenceSummary}</p><h3 className="top-space-sm">Dimension contribution cards</h3><div className="card-grid top-space-sm">{dimensionDefinitions.map(({ key, label }) => <article key={key} className="card"><p><strong>{label}</strong></p><p>Observed score: {urbanReport.dimensions?.[key]?.score ?? 'N/A'} / 5</p><p>{urbanReport.dimensions?.[key]?.narrative || 'Interpretive signal still forming.'}</p></article>)}</div><h3 className="top-space-sm">Emotional interpretation patterns</h3><p>{urbanReport.emotionalInterpretationPatterns?.join(' • ') || 'No pattern signature recorded yet.'}</p><h3 className="top-space-sm">Pressure tendencies observed</h3><p>{urbanReport.pressureTendencies?.join(' • ') || 'No pressure tendencies recorded yet.'}</p><h3 className="top-space-sm">Student-awareness indicators</h3><p>{urbanReport.studentAwarenessIndicators?.join(' • ') || 'No awareness indicators captured yet.'}</p><p className="top-space-sm"><em>Urban evidence captured — behavioral interpretation will deepen as scoring is refined.</em></p><h3 className="top-space-sm">Completion summary</h3><p>{urbanReport.completionSummary || urbanReport.evidenceSummary}</p><h3 className="top-space-sm">Student-experience interpretation</h3><p>{urbanReport.studentExperienceInterpretation || 'Interpretation is still forming as additional evidence is captured.'}</p><h3 className="top-space-sm">Leadership evidence observed</h3><p>{urbanReport.leadershipEvidenceObserved?.join(' • ') || 'No leadership evidence signatures available yet.'}</p><h3 className="top-space-sm">Growth opportunities</h3><p>{urbanReport.growthOpportunities?.join(' • ') || urbanReport.growthEdges?.join(' • ') || 'No growth opportunities identified yet.'}</p><h3 className="top-space-sm">Dashboard contribution</h3><p>{urbanReport.dashboardContribution || 'Urban evidence contributes to your blended Human Equation profile.'}</p><h3 className="top-space-sm">Timeline events</h3><p>{urbanReport.timelineEvents?.map((event) => `${event.label || event.type} (${new Date(event.occurredAt).toLocaleString('en-US')})`).join(' • ') || 'No timeline events yet.'}</p>{urbanReport.keyMoments?.length ? <details className="top-space-sm"><summary>View debug path data</summary><pre style={{ whiteSpace: 'pre-wrap' }}>{JSON.stringify(urbanReport.keyMoments, null, 2)}</pre></details> : null}</>}</div>}
