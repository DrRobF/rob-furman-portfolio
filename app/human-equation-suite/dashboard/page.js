...
import { DASHBOARD_PROFILE_STORAGE_KEY, createEmptyMasterProfile, dimensionDefinitions, toMasterProfileFromDiagnostic } from './profileData';
...
export default function HumanEquationDashboardPage() {
  ...
  useEffect(() => {
    ...
    const diagnosticRaw = window.localStorage.getItem('heq_latest_diagnostic_result_v1');
    if (!diagnosticRaw) return;
    try {
      const built = toMasterProfileFromDiagnostic(JSON.parse(diagnosticRaw));
      setProfile(built);
    ...
    <section className="section section-light">
      <div className="container">
        ...
        <div className="card project-card" style={{ border: '1px solid rgba(10,20,40,0.12)' }}>
          ...
          <div className="card-grid top-space-sm">
            ...
            <div className="card"><p><strong>Last completed simulation</strong></p><p>{profile.simulationHistory.at(-1)?.source || 'None yet'}</p></div>
      ...
      <div className="container top-space">
        ...
        <div className="card-grid top-space-sm">
          {dimensionDefinitions.map((dimension) => {
            ...
            return <article key={dimension.key} className="card project-card equal-card"><h3>{dimension.label}</h3><p>Blended composite score: {Number.isFinite(item.blendedCompositeScore) ? `${item.blendedCompositeScore} / 5` : 'Pending blend'}</p><p>{confidenceLabel[item.confidence]}</p><p>Trend: {item.trend}</p><p>Strongest evidence source: {item.strongestEvidenceSource}</p><progress max="100" value={item.statusProgress} style={{ width: '100%' }} /></article>;
          })}
        </div>
      </div>
      ...
      <div className="container top-space">
        ...
        <div className="card-grid top-space-sm">
          ...
          <article className="card"><p className="eyebrow">Currently observed</p><p>{profile.distortions.observed[0] || 'Current observation patterns are still forming.'}</p></article>
          <article className="card"><p className="eyebrow">Mild tendency</p><p>{profile.distortions.mild[0] || 'Mild tendencies are possible but unconfirmed.'}</p></article>
          <article className="card"><p className="eyebrow">Needs more evidence</p><p>{profile.distortions.needsMoreEvidence[0]}</p></article>
        </div>
      </div>
      ...
      <div className="container top-space">
        ...
        <div className="card-grid top-space-sm">
          <article className="card project-card"><h3>Timeline shell</h3><p>Diagnostic baseline captured. Additional simulations will append profile events.</p></article>
