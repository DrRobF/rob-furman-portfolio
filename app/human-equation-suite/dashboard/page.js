...
export default function HumanEquationDashboardPage() {
  ...
  const reportPanel = {
    distortions: <article className="card hes-report-card"><h2>Pressure Distortions Report</h2><div className="hes-report-grid"><section><h3>Active Distortion Tendency</h3><p>{interpretation.pressureIdentity || 'Certainty seeker'} with {interpretation.trustUnderStress || 'trust compression under ambiguity'}.</p><h4>Distortion Sequence</h4><p>Urgency → Interpretation narrowing → Trust instability → Premature action</p><h4>Recovery Interruption Points</h4><ul className="clean-list"><li>Pause and relabel threat signal before deciding.</li><li>Re-open stakeholder data before execution.</li><li>Rebuild trust signal with one clarifying leadership statement.</li></ul></section><section><h3>Trigger Map</h3><div className="hes-trigger-map">{['Ambiguity', 'Urgency', 'Emotional Escalation', 'Authority Challenge', 'Conflicting Priorities'].map((t, i) => <span key={t} style={{ '--w': `${70 - i * 8}%` }}>{t}</span>)}</div><h3>Distortion Wheel</h3><div className="hes-wheel">{['Over-controller', 'Avoider', 'Certainty seeker', 'Empathy narrowing', 'Hyper-logistics', 'Emotional accelerator'].map((risk, i) => <div key={risk} className={`slice s${i + 1}`}>{risk}</div>)}</div></section></div></article>,
    recovery: <article className="card hes-report-card"><h2>Recovery Practices Report</h2><div className="hes-report-grid"><section><h3>Recovery Ladder</h3><ol className="hes-ladder"><li><strong>Stabilize physiology:</strong> 90-second pacing reset to reduce urgency distortion.</li><li><strong>Re-anchor evidence:</strong> list verifiable facts and one unknown.</li><li><strong>Repair communication:</strong> concise transparency statement and next check-in time.</li><li><strong>Restore trust:</strong> align action promises with evidence confidence.</li></ol></section><section><h3>Stabilization Flow</h3><div className="hes-flow">Pressure <span>→</span> Distortion <span>→</span> Reset Loop <span>→</span> Stabilization</div><p><strong>Fastest recovery factor:</strong> {strongest?.label || 'Reality Anchoring'}</p><p><strong>Weakest recovery factor:</strong> {growthEdge?.label || 'Regulation Under Pressure'}</p><p><strong>Trust repair pattern:</strong> Name impact, name evidence, name next stabilizing action.</p></section></div></article>,
    timeline: <article className="card hes-report-card"><h2>Evidence Timeline Report</h2><p className="lead">Behavioral evidence progression across simulations and pressure events.</p><div className="hes-timeline">{['Diagnostic Baseline', 'Parent Call #2', 'Urban Simulation', 'Leadership Sim #4', 'Observation Lab'].map((event, i) => <article key={event}><div className="node">{i + 1}</div><div><h4>{event}</h4><p>{i === 1 ? 'Reality Anchoring increased after Parent Call #2.' : i === 2 ? 'Trust Construction dropped under high ambiguity.' : i === 3 ? 'Regulation under pressure stabilized after repeated emotional-load simulations.' : 'Evidence confidence matured through repeated exposure.'}</p></div></article>)}</div></article>,
    executive: <article className="card hes-report-card printable-report"><h2>Executive Reports</h2><p>Boardroom-grade synthesis with pressure profile, distortions, recovery recommendations, and growth trajectory.</p><div className="hes-exec-grid"><article><h3>Executive Pressure Profile</h3><p>{interpretation.summary}</p></article><article><h3>Top Stabilizers</h3><p>{interpretation.strongestCapacity || strongest?.label}</p></article><article><h3>Active Distortions</h3><p>{interpretation.pressureIdentity || 'Over-controller'} under escalating urgency.</p></article><article><h3>Growth Trajectory</h3><p>Signal maturity is rising with simulation diversity and evidence confidence.</p></article></div><div className="hes-heatmap">{sorted.map((d) => <div key={d.key}><span>{d.label}</span><b style={{ width: `${(d.blendedCompositeScore / 5) * 100}%` }} /></div>)}</div><div className="button-row top-space-sm"><button className="button secondary" onClick={() => window.print()}>Print Executive Report</button></div></article>
  };
  ...
  return <section className="section section-light"><div className="container"><LanguageSwitcher /><HumanEquationNav />
    <div className="hes-app-layout top-space-sm">
      <aside className="hes-command-sidebar">
        ...
        <section><h3>Growth Center</h3><div className="quick-actions">{growthReports.map((report) => <button key={report.id} className={`button secondary ${activeReport === report.id ? 'is-active' : ''}`} onClick={() => setActiveReport(report.id)}>{report.label}</button>)}</div></section>
      </aside>
