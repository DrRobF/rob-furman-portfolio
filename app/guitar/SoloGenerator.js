...
function chordRoot(key, semitones) {
  return chromaticKeys[(keyRootIndex[key] + semitones + 12) % 12];
}

function progressionForStyleAndKey(style, key) {
  if (style === 'Blues') return [0, 5, 0, 0, 5, 5, 0, 7].map((step) => `${chordRoot(key, step)}7`);
  if (style === 'Rock') return [`${key}5`, chordRoot(key, 3), chordRoot(key, 5), `${key}5`, `${key}5`, chordRoot(key, 3), chordRoot(key, 5), `${chordRoot(key, 7)}7`];
  if (style === 'Soulful Major') return [key, chordRoot(key, 5), chordRoot(key, 7), key, `${chordRoot(key, 9)}m`, chordRoot(key, 5), chordRoot(key, 7), key];
  return [key, chordRoot(key, 5), key, chordRoot(key, 7), chordRoot(key, 5), key, chordRoot(key, 7), key];
}
...
function eventsToTab(events) {
  ...
  events.forEach((event) => {
    ...
    const techniqueText = event.technique ?? '';
    const marker = techniqueText.includes('bend') ? 'b' : techniqueText.includes('hammer') ? 'h' : techniqueText.includes('slide') ? '/' : techniqueText.includes('vibrato') ? '~' : '';
...
function playPluckedNote(audioContext, destination, event, startTime, secondsPerBeat) {
  ...
  if (event.technique?.includes('bend') || event.marking?.includes('bend')) {
  ...
  if (event.technique?.includes('slide') || event.marking?.includes('slide')) {
  ...
  if (event.technique?.includes('hammer') || event.marking?.includes('hammer')) {
  ...
  if (event.technique?.includes('vibrato') || event.marking?.includes('vibrato')) {
    oscillator.detune.setValueAtTime(-8, startTime + Math.min(0.18, duration * 0.25));
    oscillator.detune.linearRampToValueAtTime(8, startTime + Math.min(0.32, duration * 0.45));
    oscillator.detune.linearRampToValueAtTime(-5, startTime + Math.min(0.48, duration * 0.7));
  }
...
function makeBars(style, noteEvents, barChords = progressionForStyleAndKey(style, 'A')) {
...
function makeSolo({ title, key, style, difficulty = 'Beginner', tempo, rhythmFamily = 'Sparse soulful', musicalityLevel = 'standard', notes, steps, bars }) {
  return {
    ...
    musicalityLevel,
    sourceBars: bars,
    sourceProgression: progressionForStyleAndKey(style, key),
    chordProgression: progressionForStyleAndKey(style, key),
    barChords: progressionForStyleAndKey(style, key),
    noteEvents: enhanceSoloEvents(barsToNoteEvents(bars, rhythmFamily), difficulty),
    bars: makeBars(style, enhanceSoloEvents(barsToNoteEvents(bars, rhythmFamily), difficulty), progressionForStyleAndKey(style, key)),
    totalBeats: 32,
...
function transposeSoloToKey(solo, targetKey) {
  ...
  const chordProgression = progressionForStyleAndKey(solo.style, targetKey);

  return {
    ...solo,
    id: `${solo.sourceId}-${targetKey.toLowerCase()}`,
    title: `${targetKey} ${solo.sourceTitle.replace(new RegExp(`\\b${solo.key}\\b`, 'g'), '').replace(/^(?:[A-G]\s+)+/g, '').replace(/\s+(?:in|of)\s+[A-G](?:\s|$)/g, ' ').replace(/\s{2,}/g, ' ').trim()}`,
    key: targetKey,
    chordProgression,
...
function validateSolo(solo) {
  const messages = [];
  const expectedProgression = progressionForStyleAndKey(solo.style, solo.key) ?? [];
...
const curatedSolos = [
  ...
  makeSolo({ title: 'Slow Bend Blues', key: 'A', style: 'Blues', difficulty: 'Easy', tempo: 68, rhythmFamily: 'Long-note bends', musicalityLevel: 'more', notes: ['The same bend sound returns so the solo has a hook.', 'Open spaces after each answer keep it from sounding like an exercise.', 'The final bar resolves to the major third for a clear blues ending.'], steps: ['Sing each bend before playing it.', 'Mute unused strings during the rests.', 'Play the last note longer than written.'], bars: [{ B: '-----5--------------', G: '-7b9---7-5----------', D: '-----------7--------' }, { e: '---------5----------', B: '-----5h8---8-5------', G: '---6-----------6----' }, { B: '-5-----5------------', G: '---7-5---6----------', D: '-----------7--------' }, { G: '---5-6--------------', D: '-7-----7------------' }, { B: '-------5-8-5--------', G: '---5h6-------6------' }, { e: '-5------------------', B: '---8-5--------------', G: '-------7-5----------', D: '-----------7--------' }, { G: '-5-7b8-5------------', D: '---------7----------' }, { B: '-5------------------', G: '---6-5--------------', D: '-------7------------' }] }),
  ...
  makeSolo({ title: 'Power Chord Answer', key: 'E', style: 'Rock', difficulty: 'Easy', tempo: 96, rhythmFamily: 'Syncopated pickup', musicalityLevel: 'more', notes: ['Lower-string riffs are answered by upper-string hooks.', 'Slides and a small bend add attitude without adding clutter.', 'The last bar resolves directly to the root.'], steps: ['Practice the first two bars with a metronome.', 'Let the slide in bar 4 connect smoothly.', 'Accent the first note of every two-bar phrase.'], bars: [{ D: '-2-4-5-4-2----------', A: '-----------2--------' }, { G: '-----2--------------', D: '-2-4---4-2----------' }, { B: '---3---3------------', G: '-4---4---2----------' }, { G: '-2/4-2--------------', D: '-------4-2----------' }, { G: '-2---2---4----------', D: '---4---4------------' }, { e: '-3-0----------------', B: '-----3-0------------', G: '---------2----------' }, { B: '---3-5b6-3----------', G: '-4---------4--------' }, { D: '-4-2----------------', A: '-----2--------------' }] }),
...
function enhanceSoloEvents(noteEvents, difficulty = 'Beginner') {
  const rank = difficultyRank[difficulty] ?? 0;
  const phraseShapes = [
    { starts: [0, 1.5, 3], durations: [1.15, 0.65, 0.85], name: 'question motif with space' },
    { starts: [0.5, 1.5, 2.75], durations: [0.55, 0.8, 1.1], name: 'varied motif answer' },
    { starts: [0, 1.75, 2.5], durations: [1.35, 0.5, 1.0], name: 'delayed resolution' },
    { starts: [0.75, 2, 3.25], durations: [0.55, 0.75, 0.7], name: 'anticipated response' },
    { starts: [0, 0.75, 1.5, 2.5, 3.25], durations: [0.45, 0.45, 0.55, 0.45, 0.55], name: 'intensity build' },
    { starts: [0.5, 1, 1.75, 2.5, 3.5], durations: [0.35, 0.45, 0.45, 0.5, 0.45], name: 'repeated-note burst' },
    { starts: [0, 1.5, 2.75], durations: [1.5, 0.55, 0.75], name: 'release phrase' },
    { starts: [0.5, 2.5], durations: [0.65, 1.35], name: 'confident held root' },
  ];
  const techniqueSets = [
    ['', '', 'hammer', '', 'slide'],
    ['', 'hammer', 'slide', 'bend', ''],
    ['bend', 'release', 'slide', 'double-stop', 'hammer'],
    ['expressive bend', 'pre-bend', 'slide', 'triplet burst', 'chromatic approach', 'vibrato'],
    ['vibrato', 'double-stop', 'outside passing tone', 'expressive bend', 'slide', 'syncopated stab'],
  ];
  const techniques = techniqueSets[Math.min(rank, techniqueSets.length - 1)];
  const byBar = Array.from({ length: 8 }, (_, barIndex) => noteEvents.filter((event) => event.bar === barIndex + 1));
  const enhanced = [];
  byBar.forEach((events, barIndex) => {
    const shape = phraseShapes[barIndex];
    const density = Math.min(events.length, shape.starts.length + (rank >= 2 && barIndex >= 4 && barIndex <= 5 ? 1 : 0));
    events.slice(0, density).forEach((event, eventIndex) => {
      const start = shape.starts[eventIndex % shape.starts.length];
      enhanced.push({
        ...event,
        beat: start + 1,
        startBeat: (barIndex * 4) + start,
        durationBeats: shape.durations[eventIndex % shape.durations.length] + (barIndex === 7 && eventIndex === density - 1 ? 0.55 : 0),
        technique: event.technique || techniques[(barIndex + eventIndex) % techniques.length],
        rhythmName: shape.name,
      });
    });
  });
  return enhanced;
...
function selectCuratedSolo(style, difficulty, key, previousSoloId = '') {
  ...
  const difficultyMatches = styleSolos.filter((entry) => difficultyRank[entry.difficulty] === difficultyRank[difficulty]);
