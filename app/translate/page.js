...
export default function TranslatePage() {
  ...
  return <section className={`${styles.wrapper} section`}><div className={`${styles.container} container`}><div className={styles.hero}><h1>Translate</h1><p className={styles.subtitle}>Speak naturally. Get an instant bridge. Build confidence turn by turn.</p></div>
    <div className={styles.toolbar}><select className={styles.select} value={selectedPairId} onChange={(event) => onSelectPair(event.target.value)}>{LANGUAGE_PAIRS.map((pair) => <option key={pair.id} value={pair.id}>{pair.label}</option>)}</select><button type="button" className={`button secondary ${styles.swapButton}`} onClick={swapLanguages}>Swap · {sourceLanguage} → {targetLanguage}</button></div>
    ...
    <div className={styles.actionsRow}><button type="button" className={`button secondary ${styles.clearButton}`} onClick={clearConversation}>Clear Conversation</button><button type="button" className="button primary" onClick={createLesson} disabled={!turns.length || isProcessing}>{isCreatingLesson ? 'Building lesson…' : 'Create Lesson'}</button><button type="button" className="button tertiary" onClick={() => setShowLearningLibrary((prev) => !prev)}>{showLearningLibrary ? 'Hide Learning Library' : 'Learning Library'}</button></div>
    ...
    <div className={styles.speakerDock}><div className={styles.speakerRow}>{[{ key: 'me', label: 'I’m Speaking' }, { key: 'them', label: 'They’re Speaking' }].map((speaker) => { const isActive = activeSpeaker === speaker.key; return <button key={speaker.key} type="button" className={`${styles.speakButton} ${isActive ? styles.active : ''}`} onClick={() => toggleSpeaker(speaker.key)} disabled={isProcessing}><span>{isActive ? 'Tap to Stop' : speaker.label}</span><small>{isActive ? 'Listening…' : 'Tap to start'}</small></button>; })}</div></div>
  </div></section>;
}
