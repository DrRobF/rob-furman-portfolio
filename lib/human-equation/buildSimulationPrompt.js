...
const parentLanguageMap = {
  english: 'English',
  spanish: 'Spanish',
  haitian_creole: 'Haitian Creole',
  portuguese: 'Portuguese',
};
const normalizeParentLanguage = (value) => parentLanguageMap[value] || 'English';
const scenarioTypeKeywordMap = {
  student_safety: ['fight', 'bullying', 'threat', 'safety', 'unsafe', 'injury', 'bus incident'],
  academic_conflict: ['academic', 'grade', 'grading', 'classwork', 'assignment', 'instruction', 'homework'],
  behavior_discipline: ['discipline', 'behavior', 'referral', 'suspension', 'consequence', 'phone confiscation'],
  parent_distrust: ['communication', 'ignored', 'not called', 'distrust', 'transparency'],
  staff_conduct_concern: ['teacher', 'staff', 'embarrassment', 'disrespect', 'humiliation', 'professional'],
  communication_breakdown: ['communication', 'not informed', 'follow-up', 'mixed message'],
  emotional_crisis: ['anxiety', 'panic', 'crisis', 'mental', 'emotional', 'overwhelmed', 'self-harm'],
};
...
const issueMatchesScenarioType = (issue = {}, scenarioType = '') => {
  const keywords = scenarioTypeKeywordMap[scenarioType];
...
const normalizeLeadershipSkill = (skill) => {
  if (typeof skill === 'string') return skill;
  if (!skill || typeof skill !== 'object') return '';

  const id = pickFirst(skill.cardId, skill.id) || '';
  const name = pickFirst(skill.skillName, skill.name, skill.title) || 'Unnamed skill';
  const purpose = pickFirst(skill.leadershipPurpose, skill.coreConcept, skill.description) || '';
  const pattern = pickFirst(skill.effectiveUsePattern) || '';

  const details = [purpose && `purpose: ${purpose}`, pattern && `effective pattern: ${pattern}`].filter(Boolean);
  return `${id ? `${id} ` : ''}${name}${details.length ? ` (${details.join('; ')})` : ''}`;
};
...
const normalizeTactic = (tactic) => {
  ...
  const name = pickFirst(tactic.name, tactic.tacticName, tactic.title) || 'Unnamed tactic';
  const type = pickFirst(tactic.type, tactic.tacticType) || '';
  const description = pickFirst(tactic.description, tactic.summary) || '';
  const response = pickFirst(tactic.recommendedLeadershipResponse, tactic.leadershipResponse) || '';
  const risk = pickFirst(tactic.leadershipRisk) || '';
  const timing = pickFirst(tactic.typicalTiming) || '';
  ...
  return `${name}${description ? ` — ${description}` : ''}${details.length ? ` (${details.join('; ')})` : ''}`;
};

const buildFallbackPrompt = (setup = {}) => {
  const parentLanguage = normalizeParentLanguage(setup.parentLanguage);
  return `You are roleplaying one real parent in a live school phone call. Stay emotionally realistic and responsive to the educator's tone.

${rudeEducatorGuardrail}

Parent language: ${parentLanguage}. Stay in this language unless the user explicitly requests a language change.
Do not switch languages based on background noise or overheard speech.
...
export async function buildSimulationPrompt(setup = {}) {
  try {
    ...
    const issueCandidates = normalizedIssues.filter((x) => issueMatchesGradeBand(x, setup.gradeBand) && issueMatchesScenarioType(x, setup.scenarioType));
    ...
    const selectedTactics = sampleItems(tacticDeck, Math.min(3, tacticDeck.length)).map(normalizeTactic).filter(Boolean);
    const selectedVulnerabilities = sampleItems(vulnerabilityDeck, Math.min(2, vulnerabilityDeck.length));
    const selectedLeadershipSkills = sampleItems(leadershipDeck, Math.min(3, leadershipDeck.length)).map(normalizeLeadershipSkill).filter(Boolean);
...
- ${rudeEducatorGuardrail}
- The selected scenario type is the hard reality anchor (${setup.scenarioType || 'unspecified'}). Do not replace it with a different core complaint. You may add minor realistic details only if they support the selected issue.
- When the call begins, start with one clean opening statement in the parent's selected language. Do not wait for the user to speak first if the session is connected.
- Opening should be brief and scenario-anchored. Do not start with filler-only phrases.
- Use progressive escalation pacing: open with concern, then increase pressure only if the educator is vague/dismissive/slow. Stabilize when the educator is clear and trustworthy.
- Unless the randomized context is explicitly high-chaos, do not begin at maximum intensity in the first turn.`;
