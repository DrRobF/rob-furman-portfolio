...
export async function buildSimulationPrompt(setup = {}) {
  try {
    ...
    const scenarioCandidates = scenarioPrototypes.scenarios.filter((scenario) => {
      ...
      const matchesGradeBand = !setup.gradeBand || !issue?.gradeBand || issue.gradeBand === setup.gradeBand;
      return matchesGradeBand;
    });
    const scenario = pickRandom(scenarioCandidates.length ? scenarioCandidates : scenarioPrototypes.scenarios);

    const issueCandidates = issues.issues.filter((x) => !setup.gradeBand || x.gradeBand === setup.gradeBand);
    const issue = issues.issues.find((x) => x.id === scenario.issueId) || pickRandom(issueCandidates.length ? issueCandidates : issues.issues);

    const openingArchetype = parentArchetypes.archetypes.find((x) => x.id === scenario.startingArchetypeId) || pickRandom(parentArchetypes.archetypes);
    const midCallArchetype = parentArchetypes.archetypes.find((x) => x.id === scenario.midCallArchetypeId) || pickRandom(parentArchetypes.archetypes);
    const endingArchetype = parentArchetypes.archetypes.find((x) => x.id === scenario.possibleEndingArchetypeId) || pickRandom(parentArchetypes.archetypes);
